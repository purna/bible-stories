...
Everything else in this series assumes this stage exists and works. Goal: a Vite + TypeScript scaffold with the multi-page tool architecture wired up, and a state manager that every other system (dialogue, hearing, audio, UI) reads and writes to, specifically for the Elijah game.

## 1. Scaffold

```bash
npm create vite@latest elijah-game -- --template vanilla-ts # Project name is elijah-game
cd elijah-game
npm install

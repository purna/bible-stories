...
function createContentOverlay(lineData) {
    ...
    if (lineData.text) {
        const width = lineData.width || '1/2';
        const valign = lineData.valign || 'middle';
...
async function renderLine() {
    ...
    const lineDelay = line.delay || 800;
    setTimeout(async () => {
        ...
        buildDots();
        renderChoices(line);
    }, lineDelay); // A short delay to allow the frame's pop-in animation to start.
}

...
Covers the two engines that make this a Daniel game rather than a generic top-down narrative game: the **Dialogue Engine** (ordinary scenes) and the **Vision Engine** (interpretation scenes — see `daniel-gameplay-loop.md` for the design spec this implements). Depends on `01-project-foundation.md`'s `stateManager`.

## 1. Chapter data schema

Every chapter is a data file, not hardcoded logic, so the Scene Designer tool (see `03`) can author new content without touching engine code.

```ts
// src/data/chapter.ts
import type { Reign } from "../state/stateManager";

export interface ChapterDef {
  id: number;
  title: string;
  reign: Reign | null; // null for chapters spanning no single reign's court (e.g. Ch.3)
  tilemapRef: string;
  dialogueRefs: string[];
  visionRef?: string; // present only for interpretation chapters (2, 4, 5)
  triggers: TriggerDef[];
}
...
export class DialogueEngine {
  ...
  private bindStateVariables() {
    this.story.ObserveVariable("fit_in_choice", (name, value) => {
      stateManager.write(name, value as string, this.chapter);
      if (value === true) stateManager.fitIn += 1;
      else stateManager.setApart += 1;
    });
  }

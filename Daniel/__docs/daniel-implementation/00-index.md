# Daniel — Implementation Series

A build-order companion to `daniel-game-plan.md` and `daniel-gameplay-loop.md`. Those two are the *design* docs (what the game is and why); this series is the *build* docs (how each system gets implemented in Vite/TypeScript, in the order you'd actually build it).
...
|---|-----|--------|------------|
| 01 | `01-project-foundation.md` | Vite multi-page scaffold, TypeScript config, folder structure, append-only state manager | — |
| 02 | `02-gameplay-systems.md` | Dialogue engine, Vision/Interpretation engine, chapter data schema, per-reign Favor/Suspicion | 01 |
| 03 | `03-graphics-rendering.md` | PixiJS v8 setup, tilemap renderer, character sprite pipeline, camera, scene composition | 01 |
| 04 | `04-audio-implementation.md` | Tone.js stem architecture — shared Transport clock, per-stem gain, tempo variants | 01 |
| 05 | `05-ui-design-system.md` | HUD, dialogue box, meters, typography, iconography, mobile touch layout | 03 |
| 06 | `06-ux-design-flow.md` | Onboarding, chapter transitions, save/load, pacing across a 70-year span, accessibility | 02, 05 |
| 07 | `07-post-processing-vfx.md` | PixiJS filter pipeline — bloom, vignette, color grading, dream-sequence distortion, perf budget | 03, 04 |

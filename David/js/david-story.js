/* DAVID STORY ENGINE — twelve-act layered comic.
   Stack per frame: SVG bg (parallax) → 3D midground → SVG fg → overlay.
   Data: data/manifest.json → data/actN_<id>.json
   Modeled on the Moses engine. */
(function () {
  'use strict';
...
let use3D = true;
let threeCanvas = null, renderer = null, currentScene = null, camera = null;
...
const CHARACTER_KEYS = {
  david_young: 'david_young', david: 'david', samuel: 'samuel',
  saul: 'saul', jonathan: 'jonathan', goliath: 'goliath',
  abigail: 'abigail', michal: 'michal', bathsheba: 'bathsheba',
  nathan: 'nathan', absalom: 'absalom', solomon: 'solomon',
...
function applyAtmosphere(act) {
  ...
  el('#rainLayer').classList.toggle('active', id !== 'wilderness' && id !== 'absalom' && particle !== 'storm');
  el('#seaLayer').classList.toggle('active', id === 'wilderness' || id === 'absalom' || particle === 'storm');
  el('#fireLayer').classList.toggle('active', id === 'court' || id === 'throne' || id === 'bathsheba' || id === 'census' || id === 'solomon');
  el('#thunderLayer').classList.toggle('active', id === 'wilderness' || id === 'absalom' || particle === 'storm');
  const isDawn = id === 'anointed' || id === 'solomon' || particle === 'dawn';
  const isLand = id === 'solomon';
...
async function renderLine() {
  ...
  setTimeout(async () => {
    const boxType = (data.speaker === 'narrator' || data.speaker === 'god') ? 'caption' : 'bubble';
    const effText = data.text;
    const effSpeaker = data.speaker;
    const effFx = data.fx;
    const effAlign = data.align || 'center';

    if (effSpeaker && effSpeaker !== 'narrator' && effSpeaker !== 'god') {
    ...
    const step = delays[effFx] || 18;
    const dur = (effFx === 'bounce' || effFx === 'sfx') ? 500 : (effFx === 'type') ? 200 : 400;

...
(function createJoshuaFieldBackground() {
  const field = document.querySelector('.joshua-grass-field');
  if (!field) return;
  ...
  for (let x = -3; x <= 103; x += 3 + ((x % 4) + 4) % 4) {
    ...
    clump.className = 'joshua-grass-clump';
    clump.style.left = `${x}%`;
    ...
    for (let bladeIndex = 0; bladeIndex < 3; bladeIndex += 1) {
      ...
      blade.className = 'joshua-grass-blade';

...
const Compass = (function () {
    ...
    function nudge(axis, amount) {
        ...
        else if (axis === 'peace') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        else if (axis === 'conflict') state.hasteWisdom = CLAMP(state.hasteWisdom - amount);
        else if (axis === 'unity') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        ...
        else if (axis === 'vengeance') state.hasteWisdom = CLAMP(state.hasteWisdom - amount);
        else if (axis === 'mercy') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        else if (axis === 'decisiveness') state.fearTrust = CLAMP(state.fearTrust + amount);
        else if (axis === 'hesitation') state.fearTrust = CLAMP(state.fearTrust - amount);
        else if (axis === 'gratitude') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        else if (axis === 'routine') state.hasteWisdom = CLAMP(state.hasteWisdom - amount);
        else if (axis === 'shalom') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        else if (axis === 'unrest') state.hasteWisdom = CLAMP(state.hasteWisdom - amount);
        renderIndicator();
    }

...
function loadScene3D(act, activeLine) {
  if (!use3D || !renderer || !threeCanvas || !window.createDeborahMiddleGround) return;
  disposeScene3D();
  const factory = window.createDeborahMiddleGround(act, activeLine);
  if (!factory) return;
...
function tick3D(time) {
  ...
  if (!use3D || !renderer || !currentScene || !camera) return;
  ...
  camera.lookAt(0, cfg.targetY || 0, 0);
...
function renderGame(type) {
  const area=gameArea;
  if(type==='wisdom') area.innerHTML=`<div class="choice-grid">${(currentAct().gameChoices || []).map(([answer,label])=>`<button class="choice" data-answer="${answer}">${label}</button>`).join('')}</div>`;
  if(type==='sequence') area.innerHTML=`<div class="sequence-grid">${(currentAct().gameSteps || []).map(x=>`<button class="sequence-card" data-step="${x[0]}">${x[1]}</button>`).join('')}</div>`;
...
function updateNextBtn() {
  ...
  if (atEnd && hasGame && !gameOpen && !gameCompleted[actIdx]) {
...
document.addEventListener('click', e => {
  if (!gameOpen) return;
  const retry=e.target.closest('[data-game-retry]');
  if(retry){
    const choices=[...gameArea.querySelectorAll('.choice')];
    choices.forEach(button=>{button.disabled=false;button.classList.remove('wrong')});
    gameMessage.textContent='';
    choices[0]?.focus();
    return;
  }
  ...
  if(choice){
    if(choice.dataset.answer==='right'){
      ...
      winGame(currentAct().gameSuccess || 'A faithful choice opens the way forward.');
    }else{
      choice.classList.add('wrong');
      choice.disabled=true;
      const guidance=currentAct().gameRetry || 'Review the choices, then try again.';
      gameMessage.innerHTML=`<span>${escapeHtml(guidance)}</span> <button type="button" class="game-retry" data-game-retry>Try again</button>`;
    }
  }
  ...
  if(step&&!step.classList.contains('selected')){
    ...
    if(seq.length===3){
      if(seq.join()===currentAct().gameSteps.map(step=>step[0]).join()) winGame(currentAct().gameSuccess || 'The path is complete.');
      else{
  ...
  if(ch){
    ...
    ch.disabled=true;
    tone(240);
    if([...document.querySelectorAll('[data-chariot]')].every(x=>x.disabled)) winGame(currentAct().gameSuccess || 'The tide has turned.');
  }
  ...
  if(word){
    ...
    if(picked.length===3){
      if(['Courage','Wisdom','Together'].every(x=>picked.includes(x))) winGame(currentAct().gameSuccess || 'A song worth remembering: courage, wisdom, together!');
...
async function boot() {
  ...
  await loadSvg(sceneAssetPath(STORY[0], 'background'));

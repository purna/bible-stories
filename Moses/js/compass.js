...
const Compass = (function () {
    ...
    function nudge(axis, amount) {
        ...
        else if (axis === 'provision') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        else if (axis === 'grumbling') state.hasteWisdom = CLAMP(state.hasteWisdom - amount);
        else if (axis === 'revelation') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        else if (axis === 'distraction') state.hasteWisdom = CLAMP(state.hasteWisdom - amount);
        else if (axis === 'commitment') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        else if (axis === 'lip-service') state.hasteWisdom = CLAMP(state.hasteWisdom - amount);
        else if (axis === 'zeal') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        else if (axis === 'tolerance') state.hasteWisdom = CLAMP(state.hasteWisdom - amount);
        else if (axis === 'mercy') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        else if (axis === 'wrath') state.hasteWisdom = CLAMP(state.hasteWisdom - amount);
        else if (axis === 'restoration') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        else if (axis === 'abandonment') state.hasteWisdom = CLAMP(state.hasteWisdom - amount);
        else if (axis === 'guidance') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        else if (axis === 'wandering') state.hasteWisdom = CLAMP(state.hasteWisdom - amount);
        else if (axis === 'healing') state.fearTrust = CLAMP(state.fearTrust + amount);
        else if (axis === 'death') state.fearTrust = CLAMP(state.fearTrust - amount);
        else if (axis === 'succession') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        else if (axis === 'uncertainty') state.hasteWisdom = CLAMP(state.hasteWisdom - amount);
        else if (axis === 'promise') state.fearTrust = CLAMP(state.fearTrust + amount);
        else if (axis === 'loss') state.fearTrust = CLAMP(state.fearTrust - amount);
        else if (axis === 'blessing') state.hasteWisdom = CLAMP(state.hasteWisdom + amount);
        else if (axis === 'resentment') state.hasteWisdom = CLAMP(state.hasteWisdom - amount);
        renderIndicator();
    }

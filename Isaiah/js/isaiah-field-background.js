...
(function createIsaiahFieldBackground() {
  const field = document.querySelector('.isaiah-grass-field');
  if (!field) return;
  ...
  for (let x = -3; x <= 103; x += 3 + ((x % 4) + 4) % 4) {
    ...
    clump.className = 'isaiah-grass-clump';
    clump.style.left = `${x}%`;
    ...
    for (let bladeIndex = 0; bladeIndex < 3; bladeIndex += 1) {
      ...
      blade.className = 'isaiah-grass-blade';
